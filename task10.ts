...
const users: User[] = JSON.parse(readFileSync('users2.json', 'utf8'));
...
const messageForUsers = (users: User[]) => {
  const result: Result = {};
  for (const user of users) {
    const textOfMessage = `Hello ${user.name} ${user.last_name}, please update your username "${user.username}" to be a valid email to comply with our new policy.`;
    if (user.username !== user.email) result[user.username] = { email: user.email, message: textOfMessage };
  }
  return result;
};

console.dir(messageForUsers(users));
